...
int rc_auto_loop_function_Controller1() {
  ...
  while(true) {
      ...
      // Beggining of DT strafing axis cntrl
      /*if (drivetrainStrafing < 5 && drivetrainStrafing > -5) {
        // check if the left motor has already been stopped
        if (DrivetrainStrafingNeedsToBeStopped_Controller1) {
          // stop the left drive motor
          StrafeAGroup.stop();
          StrafeBGroup.stop();
          // tell the code that the left motor has been stopped
          DrivetrainStrafingNeedsToBeStopped_Controller1 = false;
        }
      } else {
        // reset the toggle so that the deadband code knows to stop the left motor nexttime the input is in the deadband range
        DrivetrainStrafingNeedsToBeStopped_Controller1 = true;
      }*/
      // check if the value is inside of the deadband range
      ...
      /*if (DrivetrainStrafingNeedsToBeStopped_Controller1) {
        StrafeAGroup.setVelocity(drivetrainStrafing, percent);
        StrafeBGroup.setVelocity(drivetrainStrafing, percent);
