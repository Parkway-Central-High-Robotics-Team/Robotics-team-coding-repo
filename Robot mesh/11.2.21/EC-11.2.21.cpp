...
void clamp_function() {
    while(true) {
        if(con.ButtonUp.pressing()){ // == shouldnt be necessary, remove if this is the case.
...
void drivercontrol() {
    ...
    while (true) {
        ...
        omni_drive_mtr.spin(vex::directionType::fwd, omni_power, vex::velocityUnits::pct);
...
int main() {
    ...
    thread m_potthread = thread(motorTest);
    
    // Set up (but don't start) callbacks for autonomous and driver control periods.
    Competition.autonomous(autonomous); //Change THIS to switch the autonomous program we are using!!!!!!
