...
void conveyor_intake_time(double time, double velocity) {
    conveyor_mtr.spinFor(vex::directionType::rev, time, vex::timeUnits::sec, velocity, vex::velocityUnits::pct);
}

void conveyor_expel_time(double time, double velocity) {
    conveyor_mtr.spinFor(vex::directionType::fwd, time, vex::timeUnits::sec, velocity, vex::velocityUnits::pct);
}

void conveyor_intake_deg(double deg, double velocity) {
    conveyor_mtr.spinFor(vex::directionType::rev, deg, vex::rotationUnits::deg, velocity, vex::velocityUnits::pct);
}

void conveyor_expel_deg(double deg, double velocity) {
    conveyor_mtr.spinFor(vex::directionType::fwd, deg, vex::rotationUnits::deg, velocity, vex::velocityUnits::pct);
}

void conveyor_stop() {
    conveyor_mtr.stop(vex::brakeType::hold);
}
/////end of conveyor functions/////
...
void lift_up_time(double time, double velocity) {
    lift_mtr.spinFor(vex::directionType::fwd, time, vex::timeUnits::sec, velocity, vex::velocityUnits::pct);
}

void lift_down_time(double time, double velocity) {
    lift_mtr.spinFor(vex::directionType::rev, time, vex::timeUnits::sec, velocity, vex::velocityUnits::pct);
}

void liftUp_deg(double deg, double velocity){
    lift_mtr.rotateFor(vex::directionType::fwd, deg, vex::rotationUnits::deg, velocity, vex::velocityUnits::pct);
}

void liftDown_deg(double deg, double velocity){
...
//autonomus functions
void auton_test() {
    //place robot facing backwards against the wall with 3 preeloads. Make sure it is in line with with yellow mobile goal. The robot will lower the lift, go backwards, latch onto goal, pull the goal to our side, and turn on the conveyor to eject the preeloads into the mobile goal.
    liftDown_deg(290,60);
...
/////start of offical vex functions. (do not revove these)/////
void pre_auton() {
...
void drivercontrol() {
    ...
    while (true) {
        ...
        */
        ...
        //tank drive linear complex 
        /*float max = 75;
        float new_max = max/100;
        float left_percent = con.Axis3.value()*new_max;
        float right_percent = con.Axis2.value()*new_max;
        int left_power = left_percent;
        int right_power = right_percent;
        ...
        */
        ...
        float new_max = 0.5;
        left_drive_mtr.spin(vex::directionType::fwd, con.Axis3.value()*new_max, vex::velocityUnits::pct); 
        right_drive_mtr.spin(vex::directionType::fwd, con.Axis2.value()*new_max, vex::velocityUnits::pct);
...
/////end of offical vex functions. (do not revove these)/////
