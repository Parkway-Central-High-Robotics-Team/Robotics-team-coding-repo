...
void nearestGoalRight() {
    ...
    dt.driveFor(vex::directionType::fwd, 14, vex::distanceUnits::in, 50, vex::velocityUnits::pct, true);
...
void nearestGoalLeft() {
    ...
    sleepMs(2200);
    lift_mtr.stop(brakeType::hold);
    //Could be made slightly faster by starting to lower lift before moving drive train, accounting for movement forward in time to lessen the wait function.
    //Need to wait for back lift as it is against wall. 
...
void nearestGoalLeftFast() {
    
    //Brain.Screen.printLine(2, "Begin NearestGoalRight.");
    lift_mtr.spin(directionType::rev, 100, vex::velocityUnits::pct);
    right_drive_mtr.spin(directionType::fwd, 38, velocityUnits::pct);
    left_drive_mtr.spin(directionType::fwd, 38, velocityUnits::pct);
    sleepMs(2200);
    lift_mtr.stop(brakeType::hold);
    right_drive_mtr.stop(brakeType::hold);
    left_drive_mtr.stop(brakeType::hold);
...
void nearestGoalRightFast() {
    
    //Brain.Screen.printLine(2, "Begin NearestGoalRight.");
    lift_mtr.spin(directionType::rev, 100, vex::velocityUnits::pct);
    right_drive_mtr.spin(directionType::fwd, 70, velocityUnits::pct);
    left_drive_mtr.spin(directionType::fwd, 70, velocityUnits::pct);
    sleepMs(2200);
    lift_mtr.stop(brakeType::hold);
    right_drive_mtr.stop(brakeType::hold);
    left_drive_mtr.stop(brakeType::hold);
    ...
    //dt.driveFor(vex::directionType::fwd, 10, vex::distanceUnits::in, 50, vex::velocityUnits::pct, true);
    //dt.driveFor(vex::directionType::fwd, 10, vex::distanceUnits::in, 70, vex::velocityUnits::pct, true);
    right_drive_mtr.spin(directionType::fwd, 70, velocityUnits::pct);
    left_drive_mtr.spin(directionType::fwd, 70, velocityUnits::pct);
    sleepMs(1200);
    right_drive_mtr.stop(brakeType::coast);
    ...
    turnDriveTrainLeft(40); //
    sleepMs(1200);
    right_drive_mtr.stop(brakeType::coast);
    left_drive_mtr.stop(brakeType::coast);
    
    dt.driveFor(vex::directionType::fwd, 35, vex::distanceUnits::in, 50, vex::velocityUnits::pct, true);
...
void testSkills() {
    ...
    //turn to pick up our small goal which is nearby. Lower seesaw with elevator and then drive onto it holding both.
...
int main() {
    ...
    Competition.autonomous(nearestGoalRightFast); //Change THIS to switch the autonomous program we are using!!!!!!
