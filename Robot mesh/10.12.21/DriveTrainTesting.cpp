...
vex::motor      right_drive_mtr(vex::PORT2, vex::gearSetting::ratio18_1, false);
...
void limitTest() {
    while (true){
        if (limit_A == 1) {
            left_drive_mtr.spin(vex::directionType::rev, 50, vex::velocityUnits::pct);
...
void goBackToMonke() {
    while(true) {
        //con.ButtonR1.pressing( 
        if(con.ButtonR1.pressing()) {
            left_drive_mtr.spin(vex::directionType::fwd, 100, vex::velocityUnits::pct);
        }
        ...
        if(con.ButtonR2.pressing()) {
            right_drive_mtr.spin(vex::directionType::rev, 100, vex::velocityUnits::pct);
        }
...
void drivercontrol() {
    ...
    while (true) {
        ...
        thread turnIntoThread = thread(goBackToMonke);
